...
function model(; β = 0.99, s = 0.1, κ = 0.213, ι = 1.25, ε = 0.5, σ_η = 0.05, z_ss = 1,
    ρ =  0.978, σ_ϵ = 0.007, χ = 0.1, γ = 0.62, z0 = 0.0, μ_z = z0, N_z = 11, procyclical = true)
...
function optA(z, modd, Y, θ)
    ...
    a    = ~isempty(aa) ? aa[1] : 0
    y    = a*z # Expectation of y over η (given z)
    flag = ~isempty(aa) ? ((z*aa[1]/w_0 + (ψ/ε)*(hp(aa[1])*σ_η)^2) < 0) : isempty(aa) 
    flag += (w_0 < 0)
    return a, y, flag
end
...
function solveModel(modd; max_iter1 = 30, max_iter2 = 1000, max_iter3 = 1000,
    ...
    err1  = 10
    iter1 = 1
    # initialize other tolerance parameters for export
    err2  = 10
    iter2 = 1
    err3  = 10
    iter3 = 1
    ...
    @inbounds while err1 > tol1 && iter1 < max_iter1
        ...
        @inbounds while err2 > tol2 && iter2 < max_iter2   
            @inbounds for (iz,z) in enumerate(zgrid)
                az[iz], yz[iz], flag[iz] = optA(z, modd, Y_0[z0_idx], θ_0)
            end
    ...
    effort_flag = flag, exit_flag1 = (iter1 >= max_iter1), exit_flag2 = (iter2 >= max_iter2), exit_flag3 = (iter3 >= max_iter3))
end
